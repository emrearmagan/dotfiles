...
On the first turn, load the `orchestrator` skill. Don't reload — its rules stay in context.
...
- Prefer direct tools: `find`/glob for filenames, `grep` for content/regex, `read` for known files.
