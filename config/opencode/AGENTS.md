...
- Stay in scope. "In this repo" means this repo — don't read `~/.config/opencode/`, upstream docs, or global settings unless asked.

## Parallelize aggressively

opencode cannot batch tool calls in one turn — serial reads waste real time. **Default to parallel subagents whenever work is independent.**
...
Load the matching skill (see each skill's description) before tool calls. Two apply? Process first (brainstorm/debug), then implementation. Multi-intent prompts: handle sequentially in order written.
