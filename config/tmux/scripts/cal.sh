#!/bin/bash
# See: https://github.com/omerxx/dotfiles/blob/master/tmux/scripts/cal.sh
ALERT_IF_IN_NEXT_MINUTES=15
ALERT_POPUP_BEFORE_SECONDS=120
NERD_FONT_FREE="󱁕 "
NERD_FONT_MEETING="󰤙"
...
parse_attendees() {
	...
	if [[ $number_of_attendees -lt 0 ]]; then
...
print_tmux_status() {
	if [[ $minutes_till_meeting -lt $ALERT_IF_IN_NEXT_MINUTES &&
		...
		echo "#[fg=red]$NERD_FONT_MEETING \
			$time $title (${minutes_till_meeting}m)#[default]"
	else
		echo "$NERD_FONT_FREE"
	fi
...
main() {
	...
	calculate_times
	if [[ "$next_meeting" != "" && $number_of_attendees -gt 0 && $number_of_attendees -lt 2 ]]; then
		get_next_next_meeting
